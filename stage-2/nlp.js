...
export function parseNaturalLanguage(rawQuery) {
  ...
  // both present (e.g. "male and female") → no gender filter

  // ── Age group ──────────────────────────────────────────────────────────
  // "young" is special: it maps to ages 16-24, NOT to an age_group value.
  ...
  // "between X and Y" takes precedence over individual above/below
  ...
  // "aged X" – only if no other numeric range was set
  if (
    !betweenMatch &&
    !ABOVE_RE.test(q) &&
    !BELOW_RE.test(q) &&
  ...
  // ── Country ─────────────────────────────────────────────────────────────
  // Iterate longest-first so "south africa" is preferred over "africa".
